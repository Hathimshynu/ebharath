            ...
            <div class="col-md-12 p-0 record-sucess-tb">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <?php 
                        $all_transactions = $this->db->where('remark','Referral')->where('user_id',$this->session->userdata('bhpusername'))->get('wallet')->result_array();
                        
                        foreach($all_transactions as $key => $tt)
                        {
                           
                        $get_val = $this->db->select('sum(credit)-sum(debit) as credit')->where('remark','Referral')->where('id <',$tt['id'])->where('user_id',$this->session->userdata('bhpusername'))->get('wallet')->row()->credit+0;
                       //$deb = $this->db->select_sum('debit')->where('id <',$tt['id'])->where('user_id',$this->session->userdata('bhpusername'))->get('wallet')->row()->debit+0;
                         
                        $get_amt = $this->db->select('sum(credit)-sum(debit) as credit')->where('remark','Referral')->where('id <=',$tt['id'])->where('user_id',$this->session->userdata('bhpusername'))->get('wallet')->row()->credit+0;
